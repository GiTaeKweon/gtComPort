...
void CgtComPortTestDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON_SEND1, m_btnSend1);
...
void CgtComPortTestDlg::OnClear() 
{
	...
	if(TRUE == CgtComPort::checkPort(1))
		m_strEditCOM1.Empty();

	if(TRUE == CgtComPort::checkPort(2))
		m_strEditCOM2.Empty();
...
//-------------------------------------------------------------
// 전송전에 Port 의 설정을 변경시키는 경우
// 예) Bauarate : CBR_2400, CBR_9600 , ...
//-------------------------------------------------------------
