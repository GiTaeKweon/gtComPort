...
//	dcb.XoffLim = 100;
	...
	CString strTemp;
	strTemp.Format("Opened the COM %d", nPort);
...
BOOL CgtComPort::closePort()
{
	...
	CString strTemp;
	strTemp.Format("Closed the COM %d", m_nPort);
	writeMessage(strTemp);
	...
	while( TRUE )
	{
		...
		case 1: // wait comm event
			...
			if( dwCommEvent & EV_RXCHAR )
				port->readBytes(port);
				// port->readBytes(port, comstat);	// VC90 에서 오류가 발생하여 수정함
...
void CgtComPort::writeMessage(CString str)
{
...
void CgtComPort::readBytes(CgtComPort* port)
{
...
//	dcb.XoffLim = 100;
	...
	CString strTemp;
	strTemp.Format("Changed the COM %d", m_nPort);
	writeMessage(strTemp);
